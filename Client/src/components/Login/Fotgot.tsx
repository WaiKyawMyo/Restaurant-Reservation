...
import { useRequestOTPMutation } from "../../Slice/API/userApi"
import { useNavigate } from "react-router"

type Loginfrom = z.infer<typeof ForgotSchema>
...
function Forgot() {
    const navigate = useNavigate()
     ...
     const [RequestOPT ,{isLoading}] = useRequestOTPMutation()
      const Submit:SubmitHandler<Loginfrom> = async(data)=> {
         try {
           await RequestOPT(data).unwrap()
            toast.success('We’ve sent an OTP to your email.',{onClose:()=>{
                    navigate('/otpcode',{ state: { email: data.email } })
                }})
         } catch (err:any) {
